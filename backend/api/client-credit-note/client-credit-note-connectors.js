...
export default class ClientCreditnote {
  find (args = {}) {
    return client.then(db => {
      return new Promise((resolve, reject) => {
        return db.collection('client.credit.notes').find(query(args), projection)
        .toArray((err, docs) => {
          if (err) {
            reject(err);
          } else {
  ...
  findById (args = {}) {
    return client.then(db => {
      return db.collection('client.credit.notes').findOne(query(args), projection)
        .then((doc) => doc)
        .catch((err) => err.stack);
    });
  }
  ...
  findOne (args = {}) {
    return client.then(db => {
      return db.collection('client.credit.note').findOne(query(args), projection)
        .then((doc) => doc)
        .catch((err) => err.stack);
    });
  }
